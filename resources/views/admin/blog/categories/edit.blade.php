...
<div class="max-w-2xl mx-auto space-y-5">
    ...
    <form action="{{ route('admin.blog.categories.update', $category) }}" method="POST" class="bg-white rounded-2xl border p-6 space-y-4">
        @csrf @method('PUT')
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="col-span-2">
                ...
                <textarea name="description" rows="2"
                    class="w-full border rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            <div class="col-span-2 flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" id="is_active"
                    {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="rounded text-brand-600">
