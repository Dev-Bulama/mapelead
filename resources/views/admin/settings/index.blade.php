    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div id="settings-panel-integrations" style="{{ $activeTab === 'integrations' ? 'display:block' : 'display:none' }}">
            <form method="POST"
                ...
                <div>
                    <h3 class="text-sm font-semibold text-gray-800 uppercase tracking-wider mb-4 flex items-center gap-2">
                        <span class="w-6 h-6 rounded-md bg-white border border-gray-200 flex items-center justify-center">
                    ...
                    {{-- Enable Toggle --}}
                    <div class="mb-5 flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                        <div>
                            <p class="text-sm font-medium text-gray-800">Enable Google Sign-In</p>
                            <p class="text-xs text-gray-500 mt-0.5">Show "Continue with Google" button on login and register pages.</p>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer ml-4">
                            <input type="hidden" name="enable_google_login" value="0">
                            <input type="checkbox" name="enable_google_login" value="1" class="sr-only peer"
                                   {{ \App\Models\SiteSetting::get('enable_google_login', '1') ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1">From Google Cloud Console → Credentials → OAuth 2.0 Client ID.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Google Client Secret</label>
                            <input type="password"
                                   name="google_client_secret"
                                   class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono
                                          focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition"
                                   placeholder="Leave blank to keep current value">
                            <p class="text-xs text-gray-400 mt-1">Your Google OAuth client secret. Leave blank to keep the existing value.</p>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Redirect URI</label>
                            <input type="text"
                                   readonly
                                   value="{{ url('/auth/google/callback') }}"
                                   class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono bg-gray-50 text-gray-500 cursor-default">
                            <p class="text-xs text-gray-400 mt-1">Add this URI as an Authorized Redirect URI in your Google Cloud Console.</p>
                        </div>
                    </div>
                </div>
